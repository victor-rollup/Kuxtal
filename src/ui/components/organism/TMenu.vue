<script setup lang="ts">
  ...
  import TSeparator from '../atom/TSeparator.vue';
...
<template>
  <ul class="menu |" :data-orientation="props.orientation ?? 'vertical'">
    <li class="menu-item" v-for="(item, index) in props.items" @click="item.onClick" >
      <THyperlink :route="item.route" :data-use-separator="item.useSeparator" :title="item.caption" class="menu-item__hyperlink">
        <div class="menu-item__hyperlink-container">
          <component :is="item.icon" class="menu-item__icon" />
          <span class="menu-item__caption | text-truncate">{{ item.caption }}</span>
        </div>
        <TIconChevron class="menu-item__icon | menu-item__chevron" />
      </THyperlink>
      <TMenu v-if="item.submenu && item.submenu.length" :key="index" :items="item.submenu" class="submenu" />
    </li>
  </ul>
</template>
